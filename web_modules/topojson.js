...
function filterWeight(topology, minWeight, weight) {
  ...
  return function(ring, interior) {
    return weight(feature$2(topology, {type: "Polygon", arcs: [ring]}).geometry.coordinates[0], interior) >= minWeight;
  };
}
...
function presimplify(topology, weight) {
  var point = topology.transform ? transform$1(topology.transform) : copy,
      heap = newHeap();
...
export { bbox, feature, filter, filterAttached, filterAttachedWeight, filterWeight, merge, mergeArcs, mesh, meshArcs, neighbors, planarRingArea$1 as planarRingArea, planarTriangleArea, presimplify, quantile, quantize, simplify, sphericalRingArea, sphericalTriangleArea, topology, transform, untransform };
